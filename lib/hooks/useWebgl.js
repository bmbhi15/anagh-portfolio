...
export const useWebgl = () => {
  useEffect(() => {
    ...
    const displayShader = compileShader(
      gl.FRAGMENT_SHADER,
      "precision highp float; precision mediump sampler2D; varying vec2 vUv; uniform sampler2D uTexture; void main () {     vec3 c = texture2D(uTexture, vUv).rgb;     float a = max(c.r, max(c.g, c.b));     gl_FragColor = vec4(c, a); }"
    );
    ...
    let count = 0;
    let colorArr = [1, 10, 11];
    canvas.addEventListener("mousemove", function (e) {
      // count++;
      // count > 25 && ((colorArr = [1, 10, 0]), (count = 0));
      pointers[0].down = true;
      pointers[0].color = colorArr;
      pointers[0].moved = pointers[0].down;
      pointers[0].dx = (e.offsetX - pointers[0].x) * 0.05;
      pointers[0].dy = (e.offsetY - pointers[0].y) * 0.05;
    ...
    canvas.addEventListener(
      ...
      function (e) {
        ...
        for (let i = 0, len = touches.length; i < len; i++) {
          ...
          pointer.dx = (touches[i].pageX - pointer.x) * 0.05;
          pointer.dy = (touches[i].pageY - pointer.y) * 0.05;
